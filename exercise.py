...
def check_letter():
    ...
    if x in vowel: 
        ...
        print('Invalid input. Please enter a letter from (a-z or A-Z)')
...
def calculate_dog_years():
    ...
    try:
        dog_ages = int(dog_age)
        if dog_ages < 0:
            print("Age cannot be negative. Please enter a valid age.")
            
        if dog_ages <= 2:
            dog_years = dog_ages * 10
        else:
            dog_years = (2 * 10) + ((dog_ages - 2) * 7)
...
def determine_season():
    ...
    if month not in months:
        print("Invalid month. Please enter a valid month from (Jan - Dec).")
    if day < 1 or day > 31:
